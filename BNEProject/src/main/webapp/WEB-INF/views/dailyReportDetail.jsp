...
<div class="content-frame">
	...
	<div class="content-frame-body" style="height: 897px;">
		...
		<c:if test="${url eq null}">
			...
			<input type="hidden"
				value="/dailyReport/main/employee/${sessionScope.employee.employee_id}"
				id="url">
		</c:if>
				...
				<div class="panel-heading ui-draggable-handle">
					<div class="pull-left">
						...
						<h3 class="panel-title">
							...
							<!-- <small>johndoe@domain.com</small> -->
						</h3>
					</div>


					<c:if
						test="${0 eq dailyReport.approval_flag && user.position eq 'manager'}">
						<div class="pull-right" id="approvalDiv">
							<button class="btn btn-success pull-right"
								onclick="approvalDaily()">
					...
					<c:if
						test="${user.employee_id eq dailyReport.employee_id &&dailyReport.approval_flag eq 0}">
						<div class="pull-right">
							<button class="btn btn-success pull-right"
								onclick="deleteDaily(${dailyReport.daily_report_id})">
						...
						<div class="pull-right">
							<button class="btn btn-success pull-right"
								onclick="updateDaily(${dailyReport.daily_report_id})">
					...
					<div class="pull-right">
						<c:if test="${page eq null}">
							<button class="btn btn-success pull-right"
								onclick="viewList('list',1)">
						</c:if>
						<c:if test="${page ne null}">
							<button class="btn btn-success pull-right"
								onclick="viewList('list',${page})">
						</c:if>
						...
						<div class="col-md-6 col-xs-12">
							<div class="input-group">
								<span class="input-group-addon"><span
									...
									class="form-control " value="${dailyReport.reg_date}"
						...
						<div class="col-md-6 col-xs-12">
							<div class="input-group">
								<span class="input-group-addon" style="padding-bottom: 10px;"><span
									...
									class="form-control" id="aaaaa" style="cursor: default;"
									style="cursor: default;" readonly="readonly"
									value="${dailyReport.drsales}"> <span class="progress">
									<span class="progress-bar progress-bar-danger"
									role="progressbar" aria-valuenow="0" aria-valuemin="0"
									aria-valuemax="100" style="width: 0.01%" id="progressCondition">0%</span>
								</span>
						...
						<div class="col-md-2 col-xs-12">
							<div class="input-group">
								<span class="input-group-addon">출근 시 계기판</span> <input
									...
									style="cursor: default;" value="${dailyReport.before_gauge}">
							</div>
						</div>
						...
						<div class="col-md-2 col-xs-12">
							<div class="input-group">
								<span class="input-group-addon">퇴근 시 계기판</span> <input
									type="text" class="form-control" readonly="readonly"
									style="cursor: default;" value="${dailyReport.after_gauge}">
							</div>
						</div>
						...
						<div class="col-md-2 col-xs-12">
							<div class="input-group">
								<span class="input-group-addon">주행 거리</span> <input type="text"
									class="form-control" readonly="readonly"
									style="cursor: default;" style="cursor: default;"
									value="${dailyReport.after_gauge - dailyReport.before_gauge}"
									id="drivingDistance">
								...
								<ul class="list-tags">
									<c:forEach var="result" items="${counselList}">
										<li><a href="#" data-toggle="modal"
											...
											onclick="tagDetail('${result.department_name}','${result.employee_name }','${result.reg_date}','${result.title}','${result.client_id}','${result.client_name}','${result.representative}','${result.sec_client_name}','${result.address}','${result.content}')"><span
												class="fa fa-tag"></span>${result.title }</a></li>
...
"${0 eq dailyReport.approval_flag && user.position eq 'manager'}"
						...
						<div id="commentDiv">
							...
							<c:choose>
								<c:when
									test="${dailyReport.manager_comment eq null && user.position eq 'employee'}">
								</c:when>
								<c:when
									test="${dailyReport.manager_comment ne null && user.position eq 'employee'}">
								...
								<c:when
									test="${dailyReport.manager_comment eq null && user.position eq 'manager'}">
									<div class="form-group push-up-20">
										<div class="col-md-12">
											<div class="input-group">
												<input class="form-control" placeholder="팀장 의견"
													id="managerComment"> <span
													class="input-group-addon"><a href="#"
													onclick="insertComment()"><span class="fa fa-pencil"></span></a></span>
											</div>
										</div>
									</div>
								</c:when>
								<c:when
									test="${dailyReport.manager_comment ne null && user.position eq 'manager'}">
									<div class="timeline-body comments">
										<div class="comment-item">
											<img
												src="http://192.168.1.18:8085/test/${dailyReport.manager_file_position}">
		...
		<input type="hidden" value="${dailyReport.daily_report_id }"
			id="report_id"> <input type="hidden"
			value="${dailyReport.manager_name }" id="manager_name"> <input
			type="hidden" value="${dailyReport.manager_file_position }"
			id="manager_file_position">
	</div>
